  ...
  return (
    ...
    >
      <motion.div 
        ...
        className="relative w-full max-w-2xl bg-white rounded-[32px] shadow-2xl overflow-hidden border border-white/20 my-6"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header aux couleurs de l'école avec décoration */}
        <div className="bg-[#0b1a3d] text-white p-6 sm:p-8 relative overflow-hidden">
          ...
          <div className="flex items-center gap-2 mb-2">
            <span className="px-2.5 py-0.5 text-[11px] font-black uppercase rounded-md bg-[#ebe727] text-slate-950 shadow-sm border border-[#d6d21f]">
              {program.degree}
            </span>
        ...
        <div className="p-5 sm:p-6 max-h-[65vh] overflow-y-auto space-y-5 text-slate-700 text-xs sm:text-sm">
                  ...
                  <ul className="grid grid-cols-1 sm:grid-cols-2 gap-1 text-xs text-slate-600">
                    {sem.modules.map((mod, mIdx) => (
                      <li key={mIdx} className="flex items-center gap-1.5">
                        <CheckCircle className="w-3.5 h-3.5 text-[#0066bf] shrink-0" />
          ...
          <div className="bg-slate-50 border border-slate-200 rounded-xl p-3.5 flex items-center justify-between shadow-sm">
            <div className="flex items-center gap-2">
              <Coins className="w-5 h-5 text-[#0b1a3d]" />
              <div>
